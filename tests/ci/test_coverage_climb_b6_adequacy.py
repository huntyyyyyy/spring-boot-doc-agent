...
from doc_engine.ci.adequacy.criterion_ports import SLICE_KIND_MUTATOR_SURVIVORS, SLICE_KIND_STRUCTURAL
from doc_engine.ci.adequacy.mutator_survivors import MutatorSurvivorInventory
pytestmark = pytest.mark.domain_climb_sensor

def _test_github_adequacy_report_and_append_prelude(tmp_path: Path):
    ...
    (scripts / 'mutation_baseline.json').write_text('{"schema_version": 1, "accepted_survivors": {}}\n', encoding='utf-8')
    ...
    md = gas.render_adequacy_report(coverage_xml=xml, repo=tmp_path, registry_count=4, fixtures_dir=fixtures)
...
def test_mutator_enforce_and_survivors_errors(tmp_path: Path) -> None:
    ...
    bad.write_text('{"schema_version": 1, "accepted_survivors": ["not-an-object"]}\n', encoding='utf-8')
...
def test_mutator_slice_empty_names_and_path_insert(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    inventory = MutatorSurvivorInventory(registry_count=2, accepted_survivor_names=(), gate_enforce=False, assertion_enforce=False)
    ...
    stub.all_mutators = lambda: ['a', 'b', 'c']
...
def test_structural_slice_missing_and_present(tmp_path: Path) -> None:
    ...
    xml = tmp_path / 'coverage.xml'
    xml.write_text('<coverage line-rate="0.955" branch-rate="0.9"></coverage>\n', encoding='utf-8')
