...
def _test_markers_check_unknown_floor_truncation_main_prelude(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
    unknown = _seed(tmp_path, 'tests/ci/test_unknown.py', 'import pytest\npytestmark = pytest.mark.domain_ci_meta\ndef test_u():\n pass\n')
    with mock.patch.object(check_mod, 'declared_domain_markers', return_value=['domain_fake']):
        issues = check_mod.evaluate_module(tmp_path, unknown, require_classifier_match=False)
    assert any(('unknown marker' in item for item in issues))
    aligned = _seed(tmp_path, 'tests/ci/test_aligned.py', 'import pytest\npytestmark = pytest.mark.domain_ci_meta\ndef test_a():\n pass\n')
    assert check_mod.evaluate_module(tmp_path, aligned, require_classifier_match=False) == []
    _seed(tmp_path, 'tests/ci/test_ok.py', 'import pytest\npytestmark = pytest.mark.domain_ci_meta\ndef test_o():\n pass\n')
    fake_inv = DocEngineDomainInventory(floor=98.7, total=10, meeting=(), debt=tuple((Path(f'd{i}.py') for i in range(3))))
    monkeypatch.setattr(check_mod, 'build_doc_engine_inventory', lambda _repo: fake_inv)
    monkeypatch.setattr(check_mod, 'domain_path_matrix', lambda _repo: ())
    monkeypatch.setattr(check_mod, 'orphan_parallel_modules', lambda _repo: ['tests/fixtures/test_orphan.py'])
...
def test_classify_unreachable_rule_and_non_domain_values(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    call_non_attr = 'import pytest\npytestmark = mystery()\ndef test_x():\n    pass\n'
    assert classify.declared_domain_markers(call_non_attr) == []
    mixed = 'import pytest\npytestmark = [pytest.mark.domain_ci_meta, mystery(), pytest.mark.domain_schemas()]\ndef test_x():\n    pass\n'
    assert set(classify.declared_domain_markers(mixed)) >= {'domain_ci_meta', 'domain_schemas'}
...
def test_path_shards_pycache_empty_tests_and_orphan(tmp_path: Path) -> None:
    ...
    (cache / 'test_ignored.py').write_text('import pytest\npytestmark = pytest.mark.domain_ci_meta\ndef test_i():\n pass\n', encoding='utf-8')
    assert shards.discover_collection_dirs(tmp_path) == []
    _seed(tmp_path, 'tests/ci/test_gate.py', 'import pytest\npytestmark = pytest.mark.domain_ci_meta\ndef test_g():\n pass\n')
    assert shards.discover_collection_dirs(tmp_path)
    _seed(tmp_path, 'tests/fixtures/test_stray.py', 'import pytest\npytestmark = pytest.mark.domain_ci_meta\ndef test_s():\n pass\n')
...
def test_emit_abi_matrix_github_output_and_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, 'argv', ['emit_abi_matrix', '--python-versions', '3.11', '--repo-root', str(tmp_path)])
