...
def test_duration_slowest_zero_and_junit_edges(tmp_path: Path) -> None:
    report = SuiteTimingReport.from_records(
        [CaseDuration("a::b", 1.5), CaseDuration("c::d", 0.5)]
    )
    ...
    junit.write_text(
        ...
        '<testcase name="solo" time="not-a-float"/>'
        '<testcase classname="only.class" time="1"/>'
        '<testcase time="0.25"/>'
        "</testsuite>\n",
        encoding="utf-8",
    )
...
def test_timing_summary_cascade_and_append_newline(tmp_path: Path) -> None:
    ...
    # Missing junit + missing coverage → cascade then missing note.
    ...
    # Present coverage skips cascade; empty records still emit header.
    present = tmp_path / "coverage.xml"
    present.write_text('<coverage line-rate="1"/>', encoding="utf-8")
    empty = format_timing_markdown(
        SuiteTimingReport.from_records([]), top_n=3, coverage_xml=present
    )
    assert "No junit testcase durations found" in empty
    assert "Pre-pytest cascade" not in empty

    # Missing junit but present coverage → no cascade branch (line 69).
    no_cascade = render_from_junit(
        missing_junit, coverage_xml=present, top_n=1
    )
    assert "junit xml missing" in no_cascade
    assert "Pre-pytest cascade" not in no_cascade

    # Cascade branch inside format_timing_markdown when cov absent + records.
    with_records = format_timing_markdown(
        SuiteTimingReport.from_records([CaseDuration("t::x", 1.0)]),
        top_n=1,
        coverage_xml=missing_cov,
    )
    assert "Pre-pytest cascade" in with_records
    assert "1.000s" in with_records

    # append without trailing newline on prior summary.
    ...
    # append when summary file does not exist yet.
...
def test_workflow_size_yaml_absent_and_job_shapes(tmp_path: Path) -> None:
    # Under advisory: mid band already covered; hit elif skip via small file.
    ...
    # Heredoc loop continues across markers; second marker form (isolated dir).
    heredoc_dir = tmp_path / "heredoc"
    heredoc_dir.mkdir()
    (heredoc_dir / "h2.yml").write_text(
        'run: python <<PY\nprint(1)\nPY\n', encoding="utf-8"
    )
    ...
    # yaml import missing → hard diagnostic.
    with mock.patch.object(ws, "yaml", None):
        errors = ws.check_no_continue_on_error_on_reusable_call(
            small, label_fn=lambda p: p.name
        )
    assert errors and "PyYAML" in errors[0]

    # Non-dict job + uses without continue-on-error skipped; dict without uses ok.
    jobs_dir = tmp_path / "jobs"
    jobs_dir.mkdir()
    (jobs_dir / "jobs.yml").write_text(
        "name: J\non: [push]\njobs:\n"
        "  plain: 1\n"
    ...
    bad = ws.check_no_continue_on_error_on_reusable_call(
        jobs_dir, label_fn=lambda p: p.name
    )
