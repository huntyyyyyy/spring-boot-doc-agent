...
def _test_pipeline_validators_gap_review_and_main_prelude(tmp_path: Path, capsys: pytest.CaptureFixture[str]):
    ...
    problems = pv._validate_external_research(0, {'verdict': 'CONFIRMED', 'sources': [{'tier': 'C', 'url': 'https://x'}]})
    assert any(('Tier C' in p[1] for p in problems))
    problems2 = pv._validate_external_research(0, {'verdict': 'NOPE', 'sources': [{'tier': 'Z'}]})
...
def _test_pipeline_validators_gap_review_and_main_core(tmp_path: Path, capsys: pytest.CaptureFixture[str]):
    (tmp_path / 'gap_questions.json').write_text(json.dumps([{'blocks_file': 'readme', 'topic': 't', 'question': 'q?', 'evidence': 'src/A.java:1'}]), encoding='utf-8')
...
def test_pipeline_validators_gap_review_and_main(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _test_pipeline_validators_gap_review_and_main_prelude(tmp_path, capsys)
    _test_pipeline_validators_gap_review_and_main_core(tmp_path, capsys)

def test_secrets_main_and_skip_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    clean = tmp_path / 'ok.md'
    clean.write_text('no secrets here\n', encoding='utf-8')
    monkeypatch.setattr('sys.argv', ['check_no_secrets_leaked', str(clean), str(tmp_path / 'gone')])
...
def test_check_pipeline_output_main_and_ignored(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.setattr(cpo.subprocess, 'run', lambda *a, **k: SimpleNamespace(returncode=1, stdout='', stderr='boom'))
    ...
    assert cpo._resolve_dirs(SimpleNamespace(docs_dir=str(docs), target_repo=str(tmp_path / 'no-repo')))[2] == 2
...
def test_validate_artifacts_require_and_all(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    with pytest.raises(SystemExit):
        va.main(['--all', str(tmp_path), '--require', ' , , '])
    assert va.main(['--all', str(tmp_path), '--require', 'spring_signals']) == 1
    assert 'missing' in capsys.readouterr().err
    (tmp_path / 'spring_signals.json').write_text(json.dumps({'schema_version': 1, 'evidence': {}, 'config_key_sets': {}, 'entity_table_map': {}, 'scanner_version': 'sv', 'scanners': ['filesystem']}), encoding='utf-8')
