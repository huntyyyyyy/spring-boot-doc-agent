...
def _test_classify_and_citations_helpers_prelude():
    clf = drift.classify_files({'a.java': '1', 'b.java': '2', 'c.java': '3'}, {'a.java': '1', 'b.java': '9', 'd.java': '4'})
    assert clf['unchanged'] == ['a.java']
    assert clf['changed'] == ['b.java']
    assert clf['deleted'] == ['c.java']
    assert clf['added'] == ['d.java']
    signals = {'evidence': {'persistence': [{'file': 'a.java', 'line': 1, 'rule_id': 'r1'}]}, 'entity_table_map': {'Foo': {'file': 'Foo.java', 'line': 2, 'table': 'FOO'}}, 'file_signatures': {'a.java': '1'}, 'repo_path': '/r'}
...
def _test_classify_and_citations_helpers_core():
    ...
    results = []
    drift._append_uniform_status(results, [('evidence.persistence', {'file': 'a.java', 'line': 1})], 'unchanged')
    assert results[0]['status'] == 'unchanged'
    report = drift._assemble_drift_report('/repo', signals, prov, clf, list(results))
    assert report['citations_checked'] == 1
    assert report['schema_version']

def _test_stage4_pool_helpers_and_compare_prelude():
    ...
    proxy = cap.estimate_stage4_shared_pool_tokens({'groups': [{'est_tokens': 10}, {'est_tokens': 5}]}, signals_data={'n': 1})
    ...
    measured = cap.measure_stage4_shared_pool_tokens({'summaries': [1]}, interview_answers={'q': 'a'}, signals_data=None)
...
def test_empty_signatures_and_manifest_validate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with pytest.raises(SystemExit):
        drift._validate_manifest_baseline(str(tmp_path / 'm.json'), {'file_signatures': {}, 'status': 'running', 'target_repo': {'path': str(empty)}})
    drift._validate_manifest_baseline(str(tmp_path / 'm.json'), {'file_signatures': {}, 'status': 'complete', 'target_repo': {'path': str(empty)}})
