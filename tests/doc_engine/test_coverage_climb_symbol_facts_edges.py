...
def test_symbol_validation_and_package_edges() -> None:
    assert sym.ParsedSymbol(
        kind="type", namespaces=(), type_names=("Solo",)
    ).fqcn == "Solo"
...
def test_facts_maps_to_and_evidence_skips() -> None:
    contested = facts_mod._maps_to_from_contested_entry(
        "User",
        {"rule_id": "persistence__entity"},
        [{"table": "u"}, "skip-me", {"table": "v", "table_name_source": "ann"}],
        "ast-grep",
    )
    assert len(contested) == 2
    assert all(f["predicate"] == "MAPS_TO" for f in contested)

    settled = facts_mod._maps_to_from_settled_entry(
        "User",
        {"table": "users", "status": "ok", "table_name_source": "ann"},
        "ast-grep",
    )
    assert settled["qualifiers"]["status"] == "ok"
    assert facts_mod._maps_to_from_one_entry("X", "not-a-map", None) == []
    assert facts_mod._maps_to_from_one_entry(
        "X",
        {"status": "contested", "candidates": [{"table": "t"}]},
        None,
    )
    ...
    rows = facts_mod._facts_from_bucket(
        "api",
        [{"file": "A.java", "line": 1, "match": "m", "rule_id": "api__x"}],
        "ast-grep",
    )
    assert len(rows) == 1
...
def test_covering_and_recall_and_maps_to_guard(tmp_path: Path) -> None:
    ...
    assert facts_mod._astgrep_receipt_complete(
