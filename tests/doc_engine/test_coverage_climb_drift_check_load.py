...
_EMPTY_REPORT = {
    ...
    "citations_checked": 0,
    "status_counts": {},
    "file_summary": {"unchanged": [], "changed": [], "deleted": [], "added": []},
}
...
def test_load_signals_and_manifest_ok(tmp_path: Path) -> None:
    ...
    signals_path.write_text(
        json.dumps({"schema_version": 2, "file_signatures": {"a.java": "x"}, "evidence": {}}),
        encoding="utf-8",
    )
    ...
    with pytest.raises(SystemExit):
        drift._validate_manifest_baseline(
            str(man_path),
            {
                "file_signatures": {},
                "status": "complete",
                "target_repo": {"path": str(tmp_path / "missing-repo")},
            },
        )


def test_tier1_scan_walk_and_oserror(
...
def test_check_drift_fast_path_and_full(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    java = tmp_path / "a.java"
    java.write_text("class A {}\n", encoding="utf-8")
    signals = _signals_with_cite("a.java")
    signals["file_signatures"] = {
        "a.java": drift.spring_signal_scan.compute_file_signature(str(java))
    }
...
def test_cli_validate_and_main_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
        ...
        json.dumps(
            {
                "status": "complete",
                "file_signatures": {"a.java": "x"},
                "target_repo": {"path": str(repo)},
            }
        ),
        encoding="utf-8",
    )
    drift._validate_drift_cli_paths(
        SimpleNamespace(
            repo_path=str(repo), signals_path=str(signals_path), manifest=str(man)
        )
    )
    monkeypatch.setattr(drift, "check_drift", lambda *a, **k: dict(_EMPTY_REPORT))
    monkeypatch.setattr(
        "sys.argv",
        ["spring_drift_check", str(repo), str(signals_path), "--out", str(out)],
    )
    drift.main()
    ...
    monkeypatch.setattr(
        drift,
        "check_drift",
        MagicMock(side_effect=drift.spring_signal_scan.CodeQLScannerError("cq")),
    )
    ...
    monkeypatch.setattr(
        drift,
        "checked_output_path",
        MagicMock(side_effect=drift.PathValidationError("bad out")),
    )
    with pytest.raises(SystemExit):
        drift.main()
