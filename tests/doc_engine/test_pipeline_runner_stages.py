...
from tests.support.pipeline_runner.doc_writers import _write_arch, _write_doc, _write_interview, _write_summaries
pytestmark = pytest.mark.domain_pipeline

def _test_pipeline_runner_with_fixture_signals_and_mock_generative_prelude(pipeline_context):
    ...
    deterministic = [s for s in all_specs if s.kind == StageKind.DETERMINISTIC and s.name not in ('init_manifest', 'signal_scan', 'gap_probe')]
    ...
    init_runner = PipelineRunner(generative_executor=MockStageExecutor({}), stages=[init_spec])
    ...
    det_runner = PipelineRunner(generative_executor=MockStageExecutor({}), stages=deterministic)
...
def pipeline_context(tmp_path):
    ...
    docs = out / 'docs'
    docs.mkdir()
    return PipelineContext(repo_path=Path(repo), out_dir=out, manifest_path=out / 'run_manifest.json', docs_dir=docs, python=sys.executable, today='2026-07-27', respect_gitignore=False, max_tokens=120000, log=lambda _msg: None)
...
def test_subprocess_stage_runner_records_failure(pipeline_context):
    ...
    result = runner.run([sys.executable, '-c', 'import sys; sys.exit(3)'], pipeline_context)
...
def test_missing_required_output_is_stage_failure_not_crash(pipeline_context):
    ...
    spec = StageSpec(name='noop_missing_facts', kind=StageKind.DETERMINISTIC, outputs=('facts.jsonl',), argv_builder=lambda ctx: [ctx.python, '-c', 'pass'])
    runner = PipelineRunner(generative_executor=MockStageExecutor({}), stages=[spec], validate_boundaries=True)
...
def test_malformed_required_output_is_stage_failure_not_crash(pipeline_context):
    ...
    spec = StageSpec(name='noop_bad_signals', kind=StageKind.DETERMINISTIC, outputs=('spring_signals.json',), argv_builder=lambda ctx: [ctx.python, '-c', "import json, pathlib, sys; p = pathlib.Path(sys.argv[1]); p.write_text(json.dumps({'schema_version': 7}), encoding='utf-8')", str(pipeline_context.out_dir / 'spring_signals.json')])
    runner = PipelineRunner(generative_executor=MockStageExecutor({}), stages=[spec], validate_boundaries=True)
    results = runner.run(pipeline_context)
    assert len(results) == 1
    name, result = results[0]
    ...
    class FlakyManifestRunner:
        ...
        def run(self, argv, context):
            ...
            if 'end-stage' in argv:
                return StageResult(success=False, error='manifest locked', detail='end failed')
            return StageResult(success=True, detail='ok')
    spec = StageSpec(name='noop_with_manifest', kind=StageKind.DETERMINISTIC, manifest_stage='signal_scan', outputs=(), argv_builder=lambda ctx: [ctx.python, '-c', 'pass'])
    runner = PipelineRunner(subprocess_runner=FlakyManifestRunner(), generative_executor=MockStageExecutor({}), stages=[spec], validate_boundaries=False)
    results = runner.run(pipeline_context)
    assert len(results) == 1
    _, result = results[0]
    assert result.success is False
