...
def _test_query_artifacts_root_and_dispatch_prelude(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
    sig = tmp_path / 'spring_signals.json'
    sig.write_text('{}', encoding='utf-8')
    args = SimpleNamespace(kind='evidence', signals=str(sig), facts=None, run_dir=None, unsafe_no_root=False, root=None, limit=10, bucket=None, rule_id=None, file_contains=None, match_contains=None)
    ...
    assert qa._resolve_cli_root(SimpleNamespace(unsafe_no_root=False, root=str(tmp_path))) == Path(str(tmp_path))
    monkeypatch.setattr(qa, 'run_query', lambda *a, **k: {'items': []})
    assert qa._query_evidence(args, tmp_path)['items'] == []
    assert qa._query_routes(SimpleNamespace(signals=str(sig), limit=5, path_contains=None, rule_id=None, file_contains=None), tmp_path)['items'] == []
    assert qa._query_facts(SimpleNamespace(facts=str(tmp_path / 'f.jsonl'), limit=5, predicate=None, file_contains=None, fqcn=None, subject_contains=None), tmp_path)['items'] == []
...
def _test_query_artifacts_root_and_dispatch_core(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
    ...
    monkeypatch.setattr(qa, '_execute_kind', lambda *_a, **_k: (_ for _ in ()).throw(QueryError('q')))
    assert qa.main(['evidence', '--signals', str(sig)]) == 1
    monkeypatch.setattr(qa, '_execute_kind', lambda *_a, **_k: (_ for _ in ()).throw(KeyError('k')))
...
def test_cli_facade_cmds(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cli.cmd_quality_gates(SimpleNamespace(compare_ref='HEAD~1', coverage_xml=Path('c.xml'), skip_coverage=True, no_fail_fast=True)) == 0
    ...
    assert cli.cmd_coverage_gap_average(SimpleNamespace(coverage_xml=Path('c.xml'), floor=90.0, worst=5, markdown=True, append_github_summary=True)) == 0
