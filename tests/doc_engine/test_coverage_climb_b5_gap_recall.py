...
def test_recall_delta_entity_keys_and_verdicts() -> None:
    keys = rd._entity_keys(
        {
            "entity_table_map": {"Order": {"fqcn": "com.acme.Order"}},
            "entity_table_map_candidates": {"Legacy": {}},
        }
    )
    ...
    facts = rd.write_recall_miss_facts(
        signals,
        native_entity_keys=set(),
        oracle_entity_keys={"User"},
        oracle_arm="codeql",
    )
    assert facts[0]["qualifiers"]["verdict"] == "STRUCTURAL"
    assert rd.write_recall_miss_facts(
        {},
        native_entity_keys=set(),
...
def test_collect_arm_entity_keys_branches() -> None:
    native, oracle, arm = rd.collect_arm_entity_keys(
        [
            {"entity_table_map": {"A": {}}},
            {"entity_table_map": {"B": {}}},
            {"entity_table_map": {"C": {}}},
        ],
        scanner_names=["ast-grep", "codeql", "semgrep"],
    )
    assert native == {"A"}
    assert oracle == {"B"}
    assert arm == "codeql"
...
def test_symbol_collision_non_type_and_bad_map() -> None:
    ...
    contested = coll_mod.measure_r_coll(
        {
            "entity_table_map": {
                "SkipMe": "not-a-mapping",
                "Dup": {"status": "contested", "candidates": [{}, {}], "file": "D.java"},
            }
        }
    )
...
def test_join_identity_and_unmatched_edges() -> None:
    ...
    report = join_mod.measure_r_join(
        {
            "entity_table_map": {
                "Order": {
                    "fqcn": "com.acme.Order",
