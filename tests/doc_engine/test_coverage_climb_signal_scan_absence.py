...
def _test_absence_rate_block_and_measure_prelude():
    ...
    facts = [{'predicate': 'ABSENCE', 'subject': 's', 'file': 'a.java', 'qualifiers': {'trial': 'callable', 'family': 'auth'}}, {'predicate': 'ABSENCE', 'subject': 'ignored', 'file': 'z.java'}, {'predicate': 'UNPROVEN', 'subject': 'u', 'file': 'b.java'}, {'predicate': 'RECALL_MISS', 'subject': 'r', 'file': 'c.java', 'qualifiers': {'verdict': 'EVIDENTIARY', 'oracle_arm': True}}, {'predicate': 'RECALL_MISS', 'subject': 'r2', 'file': 'd.java', 'qualifiers': {'verdict': 'STRUCTURAL'}}]
...
def test_run_ast_grep_missing_and_nonzero(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sss.subprocess, 'run', lambda *a, **k: SimpleNamespace(returncode=2, stdout='', stderr='boom'))
    with pytest.raises(sss.AstGrepError, match='status 2'):
        sss.run_ast_grep('ast-grep', '.')

def test_run_ast_grep_bad_json(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sss.shutil, 'which', lambda _b: '/bin/ast-grep')
    monkeypatch.setattr(sss.subprocess, 'run', lambda *a, **k: SimpleNamespace(returncode=0, stdout='not-json', stderr=''))
...
def test_strip_and_emit_helpers(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    result = {'evidence': {'api_surface': [{'file': 'a.java'}]}, 'entity_table_map': {'A': {}}, 'redaction_zones': {'a.java': [1]}, 'config_key_sets': {'application.yml': ['a']}, 'files_scanned': 1, '_covering_proof': {'inventory_root': 'r', 'receipts': [{}]}, '_scan_partials_meta': {'x': 1}}
    ...
    monkeypatch.setattr(sss, 'fact_emit_counts', lambda _f: {'facts_total': 1, 'facts_maps_to': 0, 'facts_maps_to_contested': 0, 'facts_evidence': 1, 'facts_absence': 0, 'facts_unproven': 0, 'facts_recall_miss': 0})
...
def test_spring_signal_scan_main_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = tmp_path / 'repo'
    repo.mkdir()
    out = tmp_path / 'out' / 'signals.json'
    out.parent.mkdir()
    monkeypatch.setattr('sys.argv', ['sss', str(repo), '--out', str(out)])
    monkeypatch.setattr(sss, 'scan', lambda *a, **k: {'evidence': {}, 'entity_table_map': {}, 'redaction_zones': {}, 'config_key_sets': {}, 'files_scanned': 0, '_covering_proof': {'inventory_root': 'r', 'receipts': []}})
    monkeypatch.setattr(sss, 'write_covering_proof', lambda *a, **k: None)
    monkeypatch.setattr(sss, 'write_facts_jsonl', lambda *a, **k: None)
    monkeypatch.setattr(sss, 'facts_from_signals', lambda _r: [])
    monkeypatch.setattr(sss, 'fact_emit_counts', lambda _f: {'facts_total': 0, 'facts_maps_to': 0, 'facts_maps_to_contested': 0, 'facts_evidence': 0})
    assert sss.main() == 0
    assert out.is_file()
    monkeypatch.setattr('sys.argv', ['sss', str(tmp_path / 'missing'), '--out', str(out)])
    assert sss.main() == 1
    monkeypatch.setattr('sys.argv', ['sss', str(repo), '--out', str(out)])
    monkeypatch.setattr(sss, 'scan', lambda *a, **k: (_ for _ in ()).throw(sss.CodeQLScannerError('nope')))
    assert sss.main() == 1
...
def test_receipt_complete_helpers() -> None:
    ...
    good = {'scanner': 'filesystem', 'status': 'complete', 'expected_subset_root': 'r', 'acked_subset_root': 'r'}
