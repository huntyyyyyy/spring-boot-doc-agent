...
def _test_build_docs_site_helpers_prelude(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def _test_build_docs_site_helpers_core(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, work, work2):
    ...
    monkeypatch.setattr(site_mod.subprocess, 'run', lambda *a, **k: SimpleNamespace(returncode=1, stdout='', stderr='boom'))
...
def test_note_existing_readme_and_generative_abort(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_live_gates_main_defaults_docs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    rc = live_gates_mod.main(['--out-dir', str(tmp_path / 'out'), '--target-repo', str(tmp_path / 'repo')])
    assert rc == 0
