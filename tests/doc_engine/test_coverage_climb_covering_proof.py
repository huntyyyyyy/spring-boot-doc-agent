...
def _test_covering_build_receipt_extra_and_verify_edges_prelude():
    sigs = {'a.java': 'aa', 'b.java': 'bb'}
    root = cov.inventory_root(sigs)
    receipt = cov.build_receipt(scanner='filesystem', version_hash='v1', scope='all_signatures', expected_subset_root=root, acked_subset_root=root, status='complete', batches=2, extra={'note': 'x'})
    assert receipt['batches'] == 2 and receipt['note'] == 'x'
    ok, why = cov.verify_covering_proof(cov.build_covering_proof(file_signatures=sigs, scanner_version='sv', receipts=[receipt]), file_signatures=sigs, scanner_version='sv')
    assert ok and why == ''
    assert 'unsupported covering_proof schema_version' in (cov._schema_version_error({'schema_version': 99}) or '')
    assert cov._root_and_version_error({'inventory_root': 'nope', 'scanner_version': 'sv', 'barrier': {}}, expected_root=root, scanner_version='sv')
    assert cov._root_and_version_error({'inventory_root': root, 'scanner_version': 'other', 'barrier': {'inventory_root': root}}, expected_root=root, scanner_version='sv')
    assert cov._root_and_version_error({'inventory_root': root, 'scanner_version': 'sv', 'barrier': {'inventory_root': 'x'}}, expected_root=root, scanner_version='sv')
    assert 'receipt failed' in (cov._receipt_status_error({'status': 'failed', 'scanner': 's', 'error': 'e'}) or '')
    assert 'not complete' in (cov._receipt_status_error({'status': 'partial', 'scanner': 's'}) or '')
...
def test_covering_write_and_pop(tmp_path: Path) -> None:
    ...
    assert cov.covering_proof_path_for_signals_out(tmp_path / 'out' / 'spring_signals.json').name == 'covering_proof.json'
