---
name: fill-wave-gap
description: Close a blocks_code open question or write a six-part Quality Attribute Scenario without slipping into product code
---
...
1. Open the open question or Quality Attribute Scenario target file; quote the question.
2. Prefer writing the answer under the matching `00/`–`12/` folder
   (not only chatting).
3. If non-functional requirement-related: use `03-requirements/qas/TEMPLATE.md` six-part form.
...
6. Update open question `status:` to `SPIKE` | `CLOSED` | `WAIVED`.
7. Update `STATUS.md` and Definition of Ready row if a predicate moved.
