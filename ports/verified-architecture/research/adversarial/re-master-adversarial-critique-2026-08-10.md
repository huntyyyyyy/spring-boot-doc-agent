...
do_not:
  - Treat RE-MASTER-001 as requirements System of Record until rewritten implementation-free
  - Pin Phi-3 / any large language model identity into FR text (model choice will change)
  - Use LanceDB cosine similarity as Accept for exact symbol resolution
  - Claim FN=0 on Spring locks while excluding AOP/conditional Dependency Injection
  - Adopt Kuzu multi-instance LB architecture without Spike (embedded System of Record)
sources:
  web:
  ...
  deepwiki_ask:
    - sourcegraph/scip-java · Spring Dependency Injection / incomplete compile
...
requirements System of Record for this repo. Authoritative RE draft remains
...
conflating **verify** with **Retrieval-Augmented Generation+large language model suggest**, and using **LanceDB** where
**Source Code Index Protocol/exact graph** belongs. Model pin (Phi-3) must leave FR text entirely.
...
| Level | Evidence |
| --- | --- |
| **1** | ISO 29148:2018 / DIS Ed3 pages; Model Context Protocol 2026-07-28 Streamable HTTP + Tasks; DeepWiki Ask (scip-java, wasmtime, kuzu, tree-sitter, lancedb, scip) |
| **2** | Map draft BR/FR onto VA Layers + System of Record\|derived + this tip’s claims/coverage |
...
| Model Context Protocol 2026-07-28 **Streamable HTTP** + `Mcp-Method`/`Mcp-Name` | Normative headers exist `[Evidenced — MCP transports]` | Adopt when Phase-2 Model Context Protocol |
| Tasks as **extension** with `tasks/get` | Model Context Protocol SEP-2663 / ext-tasks `[Evidenced — MCP Tasks]` | Adopt pattern |
| non-functional requirement **reference hardware** concept | Needed for latency claims | Adopt (add Linux CI SKU) |
| WebAssembly **deny-by-default** posture for untrusted eval | Aligns Wasmtime WASI `[Evidenced — DeepWiki wasmtime]` | Pilot Could |
| Documented **risk R-01 Spring Magic** | Same Dependency Injection envelope gap as process/51 | Embody |
...
identity is already obsolete as a requirement. Remediation large language model must be an
**non-functional requirement/constraint slot** (“local inference provider meeting latency/RSS”) not a
pinned tag in FR.

### 2.2 Verify path polluted by Retrieval-Augmented Generation+large language model

OpsCon LOCAL MODE steps 4–5 put **Retrieval-Augmented Generation + Phi-3** on the critical path of
`fitness_check` (≤900 ms of the 2000 ms budget). That collapses:

| Concern | Correct layer (VA) | Draft |
| --- | --- | --- |
| Lock violation? | L2 LockCheck (deterministic) | Mixed with large language model |
| Remediation text | Optional suggest (sensor) | Mandatory FR-07 |
| Proof witness | Graph/lock IDs | Mentions Architecture Decision Record but large language model panel is core |

**Refuse:** large language model output as Accept for BR-01. Suggest may be Should/Could;
...
Source Code Index Protocol is **exact** defs/refs `[Evidenced — DeepWiki scip]`.  

Using embeddings as the primary index for lock checking is a **System of Record category
...
### 2.5 Vapor corpora as Accept System of Record
...
| Exact clause map (§5.2 nine chars, §9.3 Stakeholder Requirements Specification…) | Plausible; **not verified** against paid text in-session | Unknown |
...
| Model Context Protocol 2026-07-28 Streamable HTTP; `Mcp-Method`/`Mcp-Name` required | Primary transports page | Evidenced |
...
**Fail.** Incomplete compile → partial SemanticDB; Spring Dependency Injection richness
(`@Conditional`, AOP, SpEL) not Source Code Index Protocol’s job
...
`[Confirmed]`. Rewrite as capability policy non-functional requirement + Design adapter.
...
### 4.5 Model Context Protocol Tasks status enum (FR-15)
...
Architecture Decision Record regex `ADR-[A-Z]+-\d{3}` / `RULE-\d{4}` may not match this repo’s epic/REQ
IDs. **Every** `fitness_check` requiring violated rule + Architecture Decision Record even on **clean**
...
FR-04 duplicated mid-paste; FR-16 truncated; non-functional requirements referenced (non-functional requirement-01a…04) but
...
| --- | --- | --- |
| Tip / CI coverage System of Record | Missing | A-CI: no dual `coverage.xml` (constitution) |
...
| WebAssembly-by-default for all lock eval | **Refuse** default; **Could** sandbox (REQ-F-16) |
| Phi-3 / Ollama on verify path | **Refuse** as Must; model-agnostic Should for remediation |
| LanceDB symbol System of Record | **Refuse** |
| Org-wide Kuzu+Lance Model Context Protocol Phase 2 | **Park** — Spike; not RE System of Record |
| Backstage deferred | Aligns constitution Refuse as merge Source of Truth |
| FN=0 Spring locks | **Refuse** — use Unknown taxonomy |
| VS-* corpora Accept | **Replace** with corpus/OCS + labeled fixtures when built |
| Priority formula | **Refuse** — use MoSCoW + Requirements Traceability Matrix |
...
3. Proof artefact mandatory field *set* as non-functional requirement/schema (implementation-free).  
4. Reference hardware + **CI Linux** SKU for non-functional requirement.  
5. Model Context Protocol header/Tasks **Design constraints** when org-wide tool ships.

**Adopt (Design / Spike, not FR):**

- tree-sitter summaries; scip-java index; Wasmtime guest; Packwerk-like lock IR.  
- Language Server Protocol diagnostics shape (FR-10 intent) as Should.  
- Async task polling pattern for long tools.

**Refuse (do not copy into System of Record):**
...
- large language model on Must verify path.  
...
| --- | --- | --- |
| **CRM-1** | Park inbound draft under `docs/research/inbound/` | Path exists; status DRAFT-AI; not Design System of Record |
| **CRM-2** | Amend va RE: version envelope + SH proxies + model-agnostic remediation | REQ rows updated; Phi-* absent from FR |
| **CRM-3** | Split verify vs suggest in OpsCon | Must verify = graph+locks+receipt; large language model = Could/Should |
| **CRM-4** | Replace FN=0 with Unknown/soundness class Accept | Matches ADV-3 |
| **CRM-5** | Spike note: Kuzu embedded vs shared graph System of Record | Keep/drop for BR-04 |
...
working System of Record until a human-approved merge of Adopt items.
