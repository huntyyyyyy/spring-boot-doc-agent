---
title: Software Requirements Specification — Software Requirements Specification (MoSCoW)
status: DRAFT
date: '2026-08-10'
---
...
Implementation-free. Language choices live in Architecture Decision Records / C4, not REQ text.
...
| **REQ-F-09** | Exactly one deterministic merge-gate writer at a time (language per Architecture Decision Record) |
...
| **REQ-F-12** | Human editors get same lock violations (Language Server Protocol/diagnostics) |
...
| ID | Statement |
| --- | --- |
| **REQ-F-14** | Ghost prefetch without treating embeddings as System of Record |
| **REQ-F-15** | Cross-language impact via bridge System of Record (e.g. OpenAPI) |
| **REQ-F-16** | Capability-sandboxed lock-check guests (WebAssembly) |
| **REQ-F-20** | NL remediation suggestions via configurable local inference (not a witness) |
| **REQ-F-21** | Org-wide query API if shared-state System of Record is explicit |
...
| **REQ-F-18** | Vector/Retrieval-Augmented Generation recall as proof of a binding |
