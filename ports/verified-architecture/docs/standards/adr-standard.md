---
title: ADR standard (Nygard)
status: ACTIVE
date: '2026-08-10'
---

# Architecture Decision Record standard

**Format:** Title · Context · Decision · Status · Consequences  
**Location:** `docs/adr/adr-NNNN-slug.md`  
**Rule:** One significant decision per ADR. Supersede, do not delete.

Status vocabulary: Proposed · Accepted · Deprecated · Superseded (by ADR-xxxx).

C4 diagrams **must cite** ADR IDs for structural choices. Without them, diagrams
are sketches only.
