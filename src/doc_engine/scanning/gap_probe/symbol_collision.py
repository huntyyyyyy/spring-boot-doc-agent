...
def _contested_failure(name: str, entry: Mapping[str, Any]) -> Dict[str, Any]:
    return {
        "layer": "path_a",
        "stratum": "collision",
...
def _tally_contested(
    ...
    for name, entry in entity_map.items():
        ...
        if entry.get("status") != "contested":
            continue
        contested += 1
        failures.append(_contested_failure(name, entry))
    return contested, failures
...
def measure_r_coll(signals: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(entity_map, Mapping):
        entity_map = {}
    contested, failures = _tally_contested(entity_map)
    out = _rate_block(contested, len(entity_map))
    out["failures"] = failures
    return out
